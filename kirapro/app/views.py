...
@api_view(['GET'])
def destination_ticket_sales_report(request, city, start_date, end_date):
    tickets = Ticket.objects.filter(
        Q(train__arrival_station__city=city) &
        Q(sale_date__range=[start_date, end_date])
    )
    serializer = TicketSerializer(tickets, many=True)
    return Response(serializer.data)

@api_view(['GET'])
def train_type_ticket_sales_report(request, city, car_type):
    tickets = Ticket.objects.filter(
        Q(train__arrival_station__city=city) &
        Q(car__type=car_type)
    )
    serializer = TicketSerializer(tickets, many=True)
    return Response(serializer.data)
...
def cashier_sales_report(request, cashier_id):
    tickets = Ticket.objects.filter(cashier_id=cashier_id)
    serializer = TicketSerializer(tickets, many=True)
    return Response(serializer.data)

@api_view(['GET'])
def passenger_ticket_report(request, passenger_id):
    tickets = Ticket.objects.filter(passenger_id=passenger_id)
    serializer = TicketSerializer(tickets, many=True)
    return Response(serializer.data)
